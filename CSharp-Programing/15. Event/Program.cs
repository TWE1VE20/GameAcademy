namespace _14._Event
{
    internal class Program
    {
        ...
        public class Player
        {
            ...
            public Action OnDied;               // 이렇게 설정을 했을때 위험하다.
            ...
            {
                Console.WriteLine("플레이엉가 데미지를 받음.");
        ...
        void Main1()
        {
            ...
            // <문제점> 델리게이트를 그대로 이밴트마냥 사용할 경우 문제점
            // 1. 델리게이트 체인 = (대입) 가능
            //player.OnGetCoin = sfx.CoinSound();
            //델리게이트처럼 초기화를 해버리며는 전에 저장된 행동은 사라져버린다.

            // 2. 이밴트 발생이 외부에서 가능
            player.Died();

            // ->이를 해결하는것이 event
            // 이벤트 발생을 외부에서 못하도록 막고 이를 선언한 class 내부에서만 부를 수 있도록 막고
            // 대입 즉 = 을 이용한 최화 또한 막는다.
            // 외부에서 += -=을 이용한 추가 삭제만 가능하도록 해준다.
