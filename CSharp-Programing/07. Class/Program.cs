namespace _07._Class
{
    internal class Program
    {
        ...
        // 참조를 사용하기에 struct처럼 데이터를 가져오는것이 아닌 포인터주소로 가져온다.
        class Student
        {
            public string name;
            public int math;
        ...
        // <클래스 생성자>
        // 구조체의 경우 반환형이 없으면 초기화라고 하지만 클래스는 생성자라고 한다.
        ...
        class Car
        {
            ...
            //즉 생성자를 만들면 그 생성자만 사용 가능하고 없을 경우 자동 생성자로 만들어준다.(버전마다 다를수 있음)
            /*public Car()
            {
        ...
        class CopyConstructor
        {
            ...
            public CopyConstructor(CopyConstructor other)
            {
                ...
                //그것이 플레이어라는것만 알 필요가 있으면 이를 사용

                // 깊은복사
                this.deep = new RefType();
                this.deep.value = other.deep.value;
                //실제 변수까지 전부 알고 있어야 할 경우 이를 사용
            }
        }
